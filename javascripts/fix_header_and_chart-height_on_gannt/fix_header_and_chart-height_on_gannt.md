...
## 説明
ガントチャートの表示項目が多い場合，チャートエリアが縦に長くなるため，横スクロールバーが画面外に位置してしまいます．横スクロールバーを表示するためにはウィンドウを下方にスクロールすれば良いのですが，今度は日付ヘッダーが隠れてしまうため，日付の確認が困難になります．

この JavaScript を使用することで，チャートエリアの高さを制限し，画面内に横スクロールバーが表示されるようにします．また，スクロールした際に日付ヘッダーを画面上部に固定させることが出来るようになります．  

日付ヘッダーの固定とチャートエリアの高さ固定はチェックボックスで切り替えが可能です．  
